#!/usr/bin/env python3

import cv2
import numpy as np

class Sketcher:
    """
    A class to handle interactive drawing on images.
    Allows users to draw masks using mouse input.
    """
    
    def __init__(self, window_name, display_image, mask, brush_size=10):
        self.window_name = window_name
        self.display_image = display_image
        self.mask = mask
        self.brush_size = brush_size
        
        # Drawing state
        self.drawing = False
        self.last_point = None
        
        # Colors
        self.mask_color = 255  # White for mask
        self.display_color = (0, 255, 0)  # Green for visual feedback
    
    def mouse_callback(self, event, x, y, flags, param):
        current_point = (x, y)
        
        if event == cv2.EVENT_LBUTTONDOWN:
            # Start drawing
            self.drawing = True
            self.last_point = current_point
            self._draw_at_point(x, y)
        
        elif event == cv2.EVENT_MOUSEMOVE and self.drawing:
            # Continue drawing - draw line from last point to current point
            if self.last_point is not None:
                self._draw_line(self.last_point, current_point)
            self.last_point = current_point
        
        elif event == cv2.EVENT_LBUTTONUP:
            # Stop drawing
            self.drawing = False
            self.last_point = None
    
    def _draw_at_point(self, x, y):
        # Debug print
        print(f"Drawing at point ({x}, {y}) with brush size {self.brush_size}")
        
        # Draw filled circle on mask (white = selected)
        cv2.circle(self.mask, (x, y), self.brush_size, self.mask_color, -1)
        
        # Draw outline circle on display image for visual feedback
        cv2.circle(self.display_image, (x, y), self.brush_size, self.display_color, 2)
        
        # Debug: Check if mask was actually modified
        masked_pixels = cv2.countNonZero(self.mask)
        print(f"Debug: Mask now has {masked_pixels} white pixels")
    

    def _draw_line(self, pt1, pt2):
        # Calculate distance between points
        distance = int(np.sqrt((pt2[0] - pt1[0])**2 + (pt2[1] - pt1[1])**2))
        
        # If distance is 0, just draw at the current point
        if distance == 0:
            self._draw_at_point(pt2[0], pt2[1])
            return
        
        # Draw filled circles along the line to create brush-like stroke
        num_steps = max(distance // (self.brush_size // 3), 1)  # Adjust density as needed
        
        for i in range(num_steps + 1):
            # Interpolate between pt1 and pt2
            t = i / num_steps if num_steps > 0 else 0
            x = int(pt1[0] + t * (pt2[0] - pt1[0]))
            y = int(pt1[1] + t * (pt2[1] - pt1[1]))
            
            # Draw filled circle on mask
            cv2.circle(self.mask, (x, y), self.brush_size, self.mask_color, -1)
            
            # Draw outline circle on display image for visual feedback
            cv2.circle(self.display_image, (x, y), self.brush_size, self.display_color, -1) 

    def set_brush_size(self, size):

        self.brush_size = max(1, min(50, size))
    
    def get_brush_size(self):

        return self.brush_size
    
    def clear_drawing(self):
        self.mask.fill(0)
        self.drawing = False
        self.last_point = None
    
    def set_mask_color(self, color):

        self.mask_color = color
    
    def set_display_color(self, color):

        self.display_color = color
    
    def update_images(self, display_image, mask):

        self.display_image = display_image
        self.mask = mask
        self.drawing = False
        self.last_point = None
